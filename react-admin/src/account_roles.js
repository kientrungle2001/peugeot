...
export const AccountRoleList = (props) => (
    <List {...props} filters={<AccountRoleFilter />}>
...
const AccountRoleTitle = ({ record }) => {
    return <span>Product {record ? `"${record.title}"` : ''}</span>;
};

export const AccountRoleEdit = (props) => (
    <Edit title={<AccountRoleTitle />} {...props}>
        <TabbedForm>
...
export const AccountRoleCreate = (props) => (
    <Create {...props}>
        <TabbedForm>
            <FormTab label="Tổng quan">
                <TextInput source="title" className="ra-field-33" label="Tiêu đề" />
